...
import torchvision
from diffusionGS.models.geometry.utils import Mesh
...
class SaverMixin:
    ...
    def save_xyz_normal_points(self, filename, points) -> str:
        save_path = self.get_save_path(filename)
        if points.shape[-1] == 6:
            self.save_vertices_as_ply_open3d(points.detach().cpu().numpy()[:,:3], points.detach().cpu().numpy()[:,3:], save_path)
        else:
            self.save_vertices_as_ply_open3d(points.detach().cpu().numpy()[:,:3], None, save_path)
        return save_path

    def save_guassians_ply(self, filename, gaussians, render_video=False) -> str:
        save_path = self.get_save_path(filename)
        ...
        if render_video:
            ...
            vis_image = rearrange(vis_image, "h (v w) c -> v h w c", v=150)
            vis_image = np.ascontiguousarray(vis_image)
            imageseq2video(vis_image,save_path.replace('.ply','.mp4'), fps=30)
        return save_path


    def save_guassians_ply_scene(self, filename, gaussians, render_keyframe_c2ws = None, render_intrinsics = None, render_video=False, num_frames = 60, h=256, w=256) -> str:
        save_path = self.get_save_path(filename)
        gaussians.save_ply(save_path)
        gaussians.prune(opacity_thres=0.05).save_ply(save_path)
        if render_video:
            assert render_keyframe_c2ws is not None and render_intrinsics is not None
             # build Ks from fxfycxcy
            Ks = torch.zeros((render_keyframe_c2ws.shape[0], 3, 3), device=render_keyframe_c2ws.device)
            Ks[:, 0, 0] = render_intrinsics[:, 0]
            Ks[:, 1, 1] = render_intrinsics[:, 1]
            Ks[:, 0, 2] = render_intrinsics[:, 2]
            Ks[:, 1, 2] = render_intrinsics[:, 3]
            c2ws = torch.cat([render_keyframe_c2ws, render_keyframe_c2ws[[0], :]], dim=0)
            Ks = torch.cat([Ks, Ks[[0], :]], dim=0)
            # breakpoint()
            c2ws, Ks = get_interpolated_poses_many(
                c2ws[:, :3, :4], Ks, num_frames, order_poses=False
            )
            c2ws = c2ws.to(render_keyframe_c2ws.device)
            frame_c2ws = torch.cat([c2ws, torch.tensor([[[0, 0, 0, 1]]], device=render_keyframe_c2ws.device).repeat(c2ws.shape[0], 1, 1)], dim=1)
            frame_fxfycxcy = torch.zeros((c2ws.shape[0], 4), device=render_keyframe_c2ws.device)
            frame_fxfycxcy[:, 0] = Ks[:, 0, 0]
            frame_fxfycxcy[:, 1] = Ks[:, 1, 1]
            frame_fxfycxcy[:, 2] = Ks[:, 0, 2]
            frame_fxfycxcy[:, 3] = Ks[:, 1, 2]
            # breakpoint()
            all_frames = render_generic(gaussians, frame_c2ws, frame_fxfycxcy, h, w)
            #
            # breakpoint()
            # vis_image = rearrange(vis_image, "h (v w) c -> v h w c", v=150)
            # vis_image = np.ascontiguousarray(vis_image)
            imageseq2video(all_frames,save_path.replace('.ply','.mp4'), fps=30)
        return save_path
    ...
    def save_torch_images(self, filename, images) -> str:
        save_path = self.get_save_path(filename)
        torchvision.utils.save_image(images.detach().cpu(),save_path)
        return save_path 

    def save_torch(self, filename, input_obj) -> str:
        save_path = self.get_save_path(filename)
        torch.save(input_obj,save_path)
        return save_path 
    
    def save_vertices_as_ply_open3d(self, vertices, normal, filepath):
